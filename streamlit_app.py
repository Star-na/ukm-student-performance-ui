...
def build_model(name: str, num_classes: int, trees: int, speed_mode: str):
    ...
    if name == "SVM (RBF)":
        # SVM can be slower; keep default
    ...
    if name == "LightGBM (Main)":
        ...
        # Speed/accuracy presets (balanced by default)
        if speed_mode == "Fast":
            lr = 0.08
            leaves = 31
            depth = -1
        ...
        common = dict(
            ...
            num_leaves=leaves,
            max_depth=depth,
            min_child_samples=10,   # helps reduce "can't split" situations
            min_split_gain=0.0,
            subsample=0.9,
            colsample_bytree=0.9,
            n_jobs=-1,
            random_state=42,
            verbosity=-1,           # ✅ no spam
            force_col_wise=True     # often faster/stable on small tabular
        )
...
# Boosting tuning (only show for XGB/LGB)
is_boosting = model_name in ["XGBoost (Main)", "LightGBM (Main)"]
if is_boosting:
    speed_mode = st.sidebar.selectbox("Boosting mode", ["Balanced", "Fast", "Accurate"], index=0)
    trees = st.sidebar.slider("Number of trees (n_estimators)", 80, 350, 200, 10)
else:
    speed_mode = "Balanced"
    trees = 200
...
# =========================================================
# Session state + reset when config changes (avoid 'same result' confusion)
# =========================================================
...
if run_train:
    ...
    # ✅ Always encode labels for ALL models (fixes class mismatch forever)
    le = LabelEncoder()
