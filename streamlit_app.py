...
def build_model(name: str, num_classes: int, trees: int, speed_mode: str):
    ...
    if name == "LightGBM (Main)":
        ...
        if speed_mode == "Fast":
            lr, leaves, depth = 0.08, 31, -1
        elif speed_mode == "Accurate":
            lr, leaves, depth = 0.04, 63, -1
        else:
            lr, leaves, depth = 0.05, 31, -1
        ...
        common = dict(
            ...
            num_leaves=leaves,
            max_depth=depth,
            min_child_samples=20,     # helps reduce "no further splits" warnings
            min_split_gain=0.0,
            subsample=0.9,
            colsample_bytree=0.9,
            n_jobs=-1,
...
# =========================================================
# Session state (avoid 'same result' confusion)
# =========================================================
...
if run_train:
    ...
    with st.spinner("Training model... (Boosting may take longer on first run)"):
        try:
            ...
            # ✅ Capture model logs to prevent console spam (LightGBM warnings, etc.)
            buf = io.StringIO()
